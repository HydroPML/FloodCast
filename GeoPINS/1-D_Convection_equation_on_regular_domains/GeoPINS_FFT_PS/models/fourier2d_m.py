...
class FNN2d(nn.Module):
    def __init__(self, modes1, modes2,
        ...
        self.norm = nn.ModuleList([nn.LayerNorm(dim)
                                 for dim in self.layers[1:]])
        self.mlp_fc1 = nn.ModuleList([nn.Linear(in_size, int(in_size * 2.0))
                                 for in_size in self.layers[1:]])
        # +
        self.mlp_fc2 = nn.ModuleList([nn.Linear(int(in_size * 2.0), in_size)
                                      for in_size in self.layers[1:]])
    ...
    def forward(self, x):
        ...
        for i, (speconv, w, norm, mlp_fc1, mlp_fc2) in enumerate(zip(self.sp_convs, self.ws, self.norm, self.mlp_fc1, self.mlp_fc2)):
